...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body table-responsive">
            <table id="schedules-table" class="table table-striped table-hover table-sm text-nowrap" id="datatable">
                <thead>
                    <tr>
                        <th>Grupo</th>
                        <th>Zona</th>
        ...
        `;
            ...
            function updatePersonnelCards(group) {
                ...
                if (group.driver) {
                    ...
                    $('#driver-card').html(createPersonnelCard(group.driver.id,
                ...
                if (group.helpers && group.helpers.length > 0) {
                    ...
                    $('#helper-card-1').html(createPersonnelCard(h1.id, `${h1.names} ${h1.lastnames}`,
                        'Ayudante 1'));
                } else {
                    $('#helper-card-1').html(
                        '<div class="p-3 border rounded bg-light text-muted small text-center">Sin asignar</div>'
                ...
                if (group.helpers && group.helpers.length > 1) {
                    ...
                    $('#helper-card-2').html(createPersonnelCard(h2.id, `${h2.names} ${h2.lastnames}`,
                        'Ayudante 2'));
                } else {
                    $('#helper-card-2').html(
                        '<div class="p-3 border rounded bg-light text-muted small text-center">Sin asignar</div>'
            ...
            $('#btn-validate').on('click', function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.valid) {
                            Swal.fire('¡Éxito!',
                                'No se encontraron inconsistencias. Ya puede guardar.',
                                'success');
                    ...
                    error: function() {
                        Swal.close();
                        Swal.fire('Error', 'Hubo un problema al validar la disponibilidad.',
                            'error');
                    }
                });
            });
            ...
            $('#schedule-form').on('submit', function(e) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        table.ajax.reload();
                        Swal.fire('Guardado', 'La programación se ha registrado correctamente.',
                            'success');
                    },
                    error: function(xhr) {
            ...
            $(document).on('click', '.btn-edit', function() {
                ...
                $.get(`/admin/schedules/${id}`, function(schedule) {
                    Swal.close();
                    $('#modalScheduleLabel').html(
                        '<i class="fas fa-edit"></i> Modificar Programación');
